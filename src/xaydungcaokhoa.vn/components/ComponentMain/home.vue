<template>
  <div>
    ...
    <div v-if="categoryArticles.length > 0" >
      <large-box v-for="(item, index) of categoryArticles" :key="index" :articles="item"/>
    </div>
...
export default {
  ...
  created() {
    this.$http.get(`public/categories/wcode?type=articles`).then(res => {
     this.categoryArticles = res.body
    })
